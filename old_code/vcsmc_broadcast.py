...
class VCSMC:
    ...
    def __init__(self, datadict, K, args=None):
        ...
        self.y_q = tf.linalg.set_diag(
            tf.Variable(np.zeros((self.A, self.A)) + 1 / self.A, dtype=tf.float64, name='Qmatrix'), [0] * self.A)
        self.y_station = tf.Variable(np.zeros(self.A) + 1 / self.A, dtype=tf.float64, name='Stationary_probs')
        self.left_branches_param = tf.exp(
            tf.Variable(np.zeros(self.N - 1) + self.args.branch_prior, dtype=tf.float64, name='left_branches_param'))
        self.right_branches_param = tf.exp(
            tf.Variable(np.zeros(self.N - 1) + self.args.branch_prior, dtype=tf.float64, name='right_branches_param'))
    ...
    def compute_forest_posterior(self, data_KxXxSxA, leafnode_num_record, r):
        ...
        stationary_probs = tf.tile(tf.expand_dims(tf.transpose(self.stationary_probs), axis=0),
    ...
    def extend_partial_state_(self, JCK, potentials, map_to_indices, r):
        ...
        q_log_proposal = tf.reduce_mean(q_log_proposal,
                                        axis=1)  # q should be Kx1, but is Kx?, and reduce_mean simply changes ? to 1
    ...
    def body2_enumerate_over_topo(self, potentials, map_to_indices, core, leafnode_num_record, r, r1, r2):
        ...
        mtx_MKxSxA = self.broadcast_conditional_likelihood_K(l_data_MKxSxA, r_data_MKxSxA, l_branch_samples_MK,
                                                             r_branch_samples_MK)
    ...
    def body_rank_update(self, log_weights, log_likelihood, log_likelihood_tilde, jump_chains, jump_chain_tensor,
        ...
        remaining_core = gather_across_core(core, remaining_indices, self.N - r, self.N - r - 2,
                                            self.A)  # Kx(N-r-2)xSxA
        ...
        new_mtx_KxSxA = self.broadcast_conditional_likelihood_K(l_data_KxSxA, r_data_KxSxA, q_l_branch_samples,
                                                                q_r_branch_samples)
        new_mtx_Kx1xSxA = tf.expand_dims(new_mtx_KxSxA, axis=1)
        core = tf.concat([remaining_core, new_mtx_Kx1xSxA], axis=1)  # Kx(N-r-1)xSxA

        reamining_leafnode_num_record = gather_across_2d(leafnode_num_record, remaining_indices, self.N - r,
                                                         self.N - r - 2)
        ...
        log_weights_r = log_likelihood_r - log_likelihood_tilde - \
                        (tf.log(left_branches_param_r) - left_branches_param_r * l_branch + tf.log(
                            right_branches_param_r) - \
        ...
        log_likelihood = tf.concat([log_likelihood, [log_likelihood_r]],
                                   axis=0)  # pi(t) = pi(Y|t, b, theta) * pi(t, b|theta) / pi(Y)
    ...
    def sample_phylogenies(self):
        ...
        core_final, record_final, left_branches, right_branches, v_minus, potentials, r = tf.while_loop(
            ...
            loop_vars=[log_weights, log_likelihood, log_likelihood_tilde, self.jump_chains, self.jump_chain_tensor,
                       self.core, leafnode_num_record, left_branches, right_branches, v_minus, potentials,
                       tf.constant(0)],
